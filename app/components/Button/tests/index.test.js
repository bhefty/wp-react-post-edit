...
describe('<Button />', () => {
  ...
  it('should not adopt a type attribute when rendering a <a> tag', () => {
    const type = 'text/html'
    const renderedComponent = renderComponent({ href, type })
    expect(renderedComponent.find('a').prop('type')).toBeUndefined()
  })

  it('should not adopt a type attribute when rendering a <button> tag', () => {
    const type = 'submit'
